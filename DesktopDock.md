...
- 卡片可投放到左、中、右任意区域，拖动右下角可自由调整宽高，其他卡片会自动密排；
- 自定义宽高按紧凑和宽屏模式分别保存，卡片菜单可恢复自动大小；
...
- 公共桌面快捷方式由所有用户共享；有写入权限时会与当前用户桌面一起收纳，无权限项目保留在原位并提示失败原因。
...
- 将快捷方式拖到信息墙空白区域会先创建分类，保存名称后自动移入；在信息墙空白区域右键也可新建分类。
- “管理全部快捷方式”窗口提供完整清单、逐项分类选择和“新建分类”入口。
...
每张卡片自己的更多菜单还提供折叠、隐藏、预设尺寸、恢复自动大小、向前/向后移动；自建分类可在此删除。卡片右下角的缩放手柄用于自由调整宽高。
...
| 现象 | 原因与处理 |
| --- | --- |
| 公共桌面快捷方式仍在桌面 | 当前用户没有 Public Desktop 写入权限；DesktopDock 会保留这些项目并在收纳结果中提示失败原因 |

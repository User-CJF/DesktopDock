...
运行数据保存在当前用户 Electron `userData` 目录，不写入便携 EXE。天气是唯一自动联网模块；网页搜索只在用户主动触发时交给默认浏览器。分类、待办、随记、快捷方式和文件索引均留在本机。公共桌面快捷方式因共享权限只读展示。系统定位、通知和媒体元数据取决于 Windows 权限及具体播放器能力。
